...
class RoleRepository extends BaseRepository
{
    ...
    public function venderRole()
    {
        if (is_numeric(config('access.users.vendor_role'))) {
            return $this->query()->where('id', (int) config('access.users.vendor_role'))->first();
        }

        return $this->query()->where('name', config('access.users.vendor_role'))->first();
    }
}
