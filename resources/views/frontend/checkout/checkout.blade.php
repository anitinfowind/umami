		...
		<div class="row">
			<div class="col-lg-7 col-md-6 col-sm-12 col-12">
				...
				<div class="cho_block step1_content">
					...
					<?php if(!auth()->user()) { ?>
						<div class="row">
							<div class="col-12">
								Already have an account? <a href="{{ url('login?redirect=' . url('/checkout')) }}">Login</a>
							</div>
						</div>
					<?php } ?>
					<div class="row">
						<div class="col-lg-6 col-md-6 col-sm-6 col-12">
							<div class="form-group chk-input">
								<label>Email</label>
								<input class="form-control chk-input-style" placeholder="Email" name="email" type="text" value="{{ auth()->user() ? auth()->user()->email() : '' }}">
							</div>
						</div>
						<div class="col-lg-6 col-md-6 col-sm-6 col-12">
							<div class="form-group chk-input">
								<label>Phone</label>
								<input class="form-control chk-input-style" placeholder="Phone" name="phone" type="text" value="{{ auth()->user() ? auth()->user()->phone : '' }}">
							</div>
						</div>
					</div>
					...
					<div class="row">
						<div class="col-lg-6 col-md-6 col-sm-6 col-12">
							<div class="form-group chk-input">
								...
								<input class="form-control chk-input-style" placeholder="First Name" name="first_name" type="text" value="{{ auth()->user() ? auth()->user()->firstName() : '' }}">
							</div>
						</div>
						<div class="col-lg-6 col-md-6 col-sm-6 col-12">
							<div class="form-group chk-input">
								<label>Last Name</label>
							<input class="form-control chk-input-style" placeholder="Last Name" name="last_name" type="text" value="{{ auth()->user() ? auth()->user()->lastName() : '' }}">
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-lg-6 col-md-6 col-sm-6 col-12">
							...
							<input class="form-control chk-input-style" placeholder="Street Address" name="street_address" type="text" value="{{ isset($userAddress->id) ? $userAddress->streetAddress() : '' }}">
							</div>
						</div>
						<div class="col-lg-6 col-md-6 col-sm-6 col-12">
							<div class="form-group chk-input">
								<label>Apt / Suit / Other</label>
								<input class="form-control chk-input-style" placeholder="Apt / Suit / Other" name="address_line_2" type="text" value="{{ '' }}">
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-lg-3 col-md-6 col-sm-6 col-12">
							<div class="form-group chk-input">
								<label>City</label>
								<input class="form-control chk-input-style" placeholder="City" name="city" type="text" value="{{ $city_name }}">
							</div>
						</div>
						<div class="col-lg-3 col-md-6 col-sm-6 col-12">
							<div class="form-group chk-input">
						...
						<div class="col-lg-3 col-md-6 col-sm-6 col-12">
							<div class="form-group chk-input">
								<label>Zipcode</label>
							<input class="form-control chk-input-style number-field" placeholder="Zipcode" name="zip_code" type="text" value="{{ isset($userAddress->id) ? $userAddress->pincode : '' }}">
							</div>
						</div>
						<div class="col-lg-3 col-md-6 col-sm-6 col-12">
							<div class="form-group chk-input">
				...
				<div class="cho_block step2_content" style="display: none;">
					...
					<div class="payment-info-details">
						...
						<div class="upto767">				 
							<div class="row">
								<div class="col-12">
									<button type="button" class="btn apply-btn save-cont-btn checkout_pay">Pay {{ CURRENCY }}{{ number_format($payable_amount, 2) }}</button>
								</div>
							</div>
						</div>
					</div>
			...
			<div class="col-lg-5 col-md-6 col-sm-12 col-12">
				<div class="checkout-box positition-sticky">
					<h5>Order From <a href="{{ url('/restaurant-detail/' . $restaurant->slug) }}">{{ $restaurant->name }}</a></h5>
					...
					?>
						<div class="cho_pdbx d-flex align-items-center">
							...
							<div class="cho_price">${{ $linetotal }}</div>
						</div>
					<?php } ?>
					</div>
					<?php
					...
					<ul class="food-price-sec pt-0">
						<li class="d-flex justify-content-between align-items-center"><label>Subtotal</label><span>{{CURRENCY}}{{ number_format($total_cart_price,2) }}</span></li>
						<?php
						...
						?>
						<li class="d-flex justify-content-between align-items-center"><label>Processing Fee</label><span>{{CURRENCY}}{{ number_format($processing_fee,2) }}</span></li>
						<li class="d-flex justify-content-between align-items-center shipping_charge"><label>Shipping Charge</label><span price="0.00">{{CURRENCY}}0.00</span></li>
						<li class="d-flex justify-content-between align-items-center"><label>Payable Amount</label><span class="payable_ammount" price="{{ number_format($payable_amount, 2) }}">{{CURRENCY}}{{ number_format($payable_amount, 2) }}</span></li>
			      ...
			      $estimated_delivery_date = estimated_delivery_date(['restaurant' => $restaurant]);
						...
						$dp_format_date = date('m/d/Y', strtotime($estimated_delivery_date['delivery_date']));
			      		?>
			      		<!-- <li class="d-flex justify-content-between align-items-center"><label>Estimated Delivery Date <a href="javascript:;" class="change_delivery_date" delivery_date="{{ $estimated_delivery_date['delivery_date'] }}"><i class="fa fa-edit"></i></a></label><span>{{ $delivery_date }}</span></li> -->
					</ul>

					<?php if(auth()->user()) { ?>
						<?php if($earnable_points > 0) { ?>
							<div class="reward_point_info text-center" reward_point="{{ $earnable_points }}"><cite>You will earn <b>{{ $earnable_points }}</b> reward points with this order</cite></div>
						<?php } ?>
					<?php } ?>
					<br>
					...
					<div class="hobb">
						...
						<p><a href="javascript:;" class="change_delivery_date" delivery_date="{{ $estimated_delivery_date['delivery_date'] }}"><span>{{ $delivery_date }}</span> <i class="fa fa-calendar"></i></a></p> -->

						<h5>Estimated Delivery Date <a href="javascript:;" class="change_delivery_date" delivery_date="{{ $estimated_delivery_date['delivery_date'] }}"><span>{{ $delivery_date }}</span> <i class="fa fa-calendar"></i></a></h5>
					</div>
  ...
  return {'error': error, 'field_data': {'first_name': first_name, 'last_name': last_name, 'country_id': country_id, 'country': country, 'street_address': street_address, 'address_line_2': address_line_2, 'alternative_address': alternative_address, 'state_id': state_id, 'state': state, 'city': city, 'zip_code': zip_code, 'email': email, 'phone': phone, 'is_gift': is_gift, 'gift_message': gift_message, 'gift_message_name': gift_message_name, 'card_number': card_number, 'card_exp_month': card_exp_month, 'card_exp_year': card_exp_year, 'card_cvc': card_cvc, 'restaurant_id': restaurant_id, 'delivery_date': delivery_date}};
	...
	$(document).ready(function(){
		...
		$(".delivery_datepicker").datepicker({
			firstDay: 1,
			minDate: '{{ $dp_format_date }}',
			beforeShowDay: function(date) {
		        var day = date.getDay();
		        var dys = '{{ implode(',', $days) }}';
		        dys = dys.split(',');
