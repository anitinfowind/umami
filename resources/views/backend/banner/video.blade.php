                ...
                <table id="example" class="table table-condensed table-hover table-bordered">
                    ...
                    <tbody>
                        @if($banners->isNotEmpty())
                            ...
                            <?php
                             $info = pathinfo(public_path('/uploads/banner/').$banner->video);
                                $ext = $info['extension'];
                                ?>
                                <tr>
                                    <td>
                                     @if($ext=='jpeg'||$ext=='jpg'||$ext=='png' ||$ext=='gif'||$ext=='webp')
                                        @if(!empty($banner->video) && File::exists(public_path('/uploads/banner/').$banner->video))
                                        <img style="width: 70px" src="{{URL::to('uploads/banner/'.$banner->video)}}">
                                        @else
                                        <img style="width: 70px" src="{{URL::to('images/no-image.png')}}">
                                        @endif
                                      @else
                                       <video width="150px" height="150px" muted loop  controls> 
                                        <source src="{{URL::to('public/uploads/banner/'.$banner->video)}}" type="video/mp4">
                                        </video>
                                      @endif
                                    </td>
                                    <td>{{ $banner->title() }}</td>
                                    ...
                                    <td>
                                        <a href="{{ route("admin.banner.videoedit", $banner->id()) }}" class="btn btn-flat btn-default">
