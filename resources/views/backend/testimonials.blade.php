              ...
              <table id="example" class="table table-condensed table-hover table-bordered" data-order111="[[ 3, &quot;desc&quot; ]]">
                  ...
                  <tbody>
                        @foreach($testimonials as $tst)
                          <tr>
                            <td>{!! $tst->image != '' ? '<img src="' . url('public/uploads/testimonial/' . $tst->image) . '" style="height: 80px;" />' : '' !!}</td>
                            <td>{{ $tst->first_name . ' ' . $tst->last_name }}</td>
                            <td>{{ $tst->title }}</td>
                            <td>{!! nl2br($tst->comment) !!}</td>
                            <td>{!! $tst->status == '1' ? '<i class="fa fa-check"></i>' : '' !!}</td>
                              <td>
                                ...
                                <li><a class="edit_testimonial" href="javascript:;" testimonial_id="{{ $tst->id }}"><i class="fa fa-pencil" data-toggle="tooltip" data-placement="top"></i>Edit</a></li>
                                <li><a class="delete_testimonial text-danger" href="javascript:;" testimonial_id="{{ $tst->id }}"><i class="fa fa-trash" data-toggle="tooltip" data-placement="top"></i>Delete</a></li>
                                </ul>
                               </div>
                              </td>
                          </tr>
      ...
      $(document).ready(function() {
          ...
          $.ajax({
            ...
            success: function (data) {
              ...
              if(testimonial.image != '' && testimonial.image != null) {
                $('#myModal .imgBlock img').attr('src', '{{ url('public/uploads/testimonial') }}/' + testimonial.image);
              ...
              if(testimonial.post_image != '' && testimonial.post_image != null) {
                if(myvar !='' && myvar !=null || myvar == 'mp4' || myvar == 'ogx' || myvar == 'oga' || myvar == 'ogv' || myvar == 'ogg' || myvar == 'webm' ){
                  var url = '{{ url('public/uploads/testimonial/post_image') }}'+testimonial.post_image;
                ...
                //$('#myModal .imgBlockPost img').attr('src', '{{ url('public/uploads/testimonial/post_image') }}/' + testimonial.post_image);
        ...
        $(document).on('click', '.delete_testimonial', function(){
          ...
          if(confirm('Are you sure to delete this testimonial?')) {
            var url = '{{ url('admin/delete_testimonial') }}';
            var data = new FormData();
            data.append('testimonial_id', testimonial_id);
            data.append('_token', $('meta[name="csrf-token"]').attr('content'));
