...
@section ('title', trans('Products Management') . ' | ' . trans('labels.backend.products.edit'))

@section('page-header')
    <h1>
        {{ trans('Products Management') }}
        <small>{{ trans('labels.backend.products.edit') }}</small>
    </h1>
@endsection

@section('content')
    {{ Form::model($products, ['route' => ['admin.products.update', $products], 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'PATCH', 'id' => 'edit-product','files' => true,]) }}
